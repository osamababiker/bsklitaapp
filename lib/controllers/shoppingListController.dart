...
class ShoppingListDbController extends GetxController {
  static final ShoppingListDbController _instance =
      ShoppingListDbController.internal();

  factory ShoppingListDbController() => _instance;
    ...
    try {
      ...
      if (maps.isNotEmpty) {
        for (int i = 0; i < maps.length; i++) {
          items.add(ShoppingList.fromJson(maps[i]));
        }
      }
      isLoading(false);
  ...
  Future<double> getListTotal() async {
    ...
    try {
      ...
      for (var i = 0; i < listData.length; i++) {
        ShoppingList list = ShoppingList.fromJson(listData[i]);
        total += list.price!.toDouble() * list.quantity;
      }
    } catch (e) {
      print("problem selecting all items == $e");
      throw (e);
